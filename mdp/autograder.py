...
input_file_ls = ['data/continuing-mdp-10-5.txt','data/continuing-mdp-2-2.txt', 'data/continuing-mdp-25-10.txt', 'data/continuing-mdp-50-20.txt','data/episodic-mdp-10-5.txt','data/episodic-mdp-2-2.txt', 'data/episodic-mdp-25-10.txt', 'data/episodic-mdp-50-20.txt']
flag_ok = 0
    ...
    def __init__(self,algorithm,print_error):
        ...
        for algo in algorithm_ls:
            ...
            for in_file in input_file_ls:
                print("\n\n","-"*100)
                if algo == 'default':
                    cmd_planner = r"python","dynamic_programming.py","--mdp",in_file
                else:
                    cmd_planner = r"python","dynamic_programming.py","--mdp",in_file,"--algorithm",algo
        ...
        policy_eval_files = ['data/continuing-mdp-10-5.txt', 'data/episodic-mdp-10-5.txt']
        for in_file in policy_eval_files:
            cmd_planner = r"python","dynamic_programming.py","--mdp",in_file, "--policy", in_file.replace("continuing","policy-continuing").replace("episodic","policy-episodic")
...
def run(gameconfig, test):
    cmd_encoder = r"python", "encoder.py", "--game_config", gameconfig
    print("\n","Generating the MDP encoding using encoder.py")
    f = open('verify_attt_mdp','w')
    subprocess.call(cmd_encoder,stdout=f)
    f.close()

    cmd_planner = r"python","dynamic_programming.py","--mdp","verify_attt_mdp"
    print("\n","Generating the value policy file using dynamic_programming.py using default algorithm")
    f = open('verify_attt_planner','w')
    subprocess.call(cmd_planner,stdout=f)
    f.close()

    cmd_decoder = r"python","decoder.py","--value_policy","verify_attt_planner","--testcase",test
...
if __name__ == "__main__":
    ...
    if(args.task == 'dp'):
        ...
        if(flag_ok):
            print("THERE IS A MISTAKE in DP Task")
    else:
        print("Running DP task")
        print("\n\n","-"*100)
        print("DP TASK")
        print("\n\n","-"*100)
        algo = VerifyOutputPlanner(args.algorithm,args.pe)
